...
class Step {
  ...
  public static function showUrl () {
    echo "\n";
    if (USNAME) echo " " . self::color ('➜', 'R') . " " . self::color ('您的網址是', 'G') . "：" . self::color (PROTOCOL . BUCKET . '/' . NAME . '/', 'W') . "\n\n";
    else echo " " . self::color ('➜', 'R') . " " . self::color ('您的網址是', 'G') . "：" . self::color (PROTOCOL . BUCKET . '/', 'W') . "\n\n";
    echo str_repeat ('=', 80) . "\n";
  }
  ...
  public static function filterLocalFiles () {
    ...
    $files = array_filter (Step::$localFiles, function ($file) {
      foreach (Step::$s3Files as $s3File)
        if (USNAME && ($s3File['name'] == (NAME . DIRECTORY_SEPARATOR . $file['uri'])) && ($s3File['hash'] == $file['md5']))
            return false;
        else if (($s3File['name'] == ($file['uri'])) && ($s3File['hash'] == $file['md5']))
            return false;
