...
$(function () {
  ...
  window.vars.$a = $('#menus a').click (function () {
    ...
    window.vars.$count.text ($(this).index () + 1 + '/' + window.vars.$a.length).addClass ('s');
  ...
  window.funs.updateVal = function ($form, type, val, callback) {
    ...
    .done (function (result) {
      if (result.gym || result.store) window.vars.$a.filter ('[data-id="' + $form.data ('info').id + '"]').addClass ('ed');
       else window.vars.$a.filter ('[data-id="' + $form.data ('info').id + '"]').removeClass ('ed');
      
      $tip.text ('更新成功').removeClass ('r');
    })
